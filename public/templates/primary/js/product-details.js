        ...
        if (buyNowBtn) {
            buyNowBtn.addEventListener('click', () => {
                ...
                if (product.hasVariations) {
                    ...
                    if (product.stocks < 1) {
                        window.showNotification('This product is out of stock', 'error');
                        return;
                      }
...
async function fetchProductBasic(productId) {
    try {
        ...
        const response = await fetch(`${this.baseUrl}/public/products/${productId}?subdomain=${subdomain}`);
...
function addEventListenersBasic(product) {
    // Implementation matches your original addEventListeners function
    ...
    if (addToCartBtn && !addToCartBtn.disabled) {
        addToCartBtn.addEventListener('click', () => {
            if (product.hasVariations) {
                // Show variation modal if product has variations
                if (window.cart && typeof window.cart.showVariationModal === 'function') {
                    window.cart.showVariationModal(product);
                } else {
                    console.error('Cart variation modal not available');
    ...
    if (buyNowBtn && !buyNowBtn.disabled) {
        buyNowBtn.addEventListener('click', () => {
            if (product.hasVariations) {
                ...
                if (window.cart && typeof window.cart.showVariationModal === 'function') {
                    // Set a flag to indicate this is buy now
