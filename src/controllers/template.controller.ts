  ...
  try {
    ...
    try {
      ...
      if (
        ...
        const pageName = path
          .basename(fileInfo.filePath, ".html")
          .toLowerCase();
        fileContent = await injectPageAssets(
          fileContent,
          templateDir,
          pageName
        );
 ...
 * @returns The HTML with all assets injected
 */
async function injectPageAssets(
  htmlContent: string,
  templateDir: string,
  pageName: string
): Promise<string> {
  ...
  try {
    ...
    // Check for page-specific CSS in the css directory
    const pageCssFilesInDir = allFiles.filter((file) => {
      const normalizedPath = file.toLowerCase();
      const basename = path.basename(normalizedPath);
      return (
        normalizedPath.includes("/css/") &&
        (basename === `${pageName}.css` ||
          basename === `${pageName}-styles.css`)
      );
    });

    for (const cssFile of pageCssFilesInDir) {
      cssFiles.push({
        path: cssFile,
        content: await fs.readFile(cssFile, "utf-8"),
      });
    ...
    // Check for common CSS files in the css directory
    ...
    for (const cssFileName of commonCssFiles) {
      const cssFilePaths = allFiles.filter((file) => {
        ...
        return (
          normalizedPath.includes("/css/") &&
          path.basename(normalizedPath) === cssFileName.toLowerCase()
        );
      });
    ...
    // Check for page-specific JS in the js directory
    const pageJsFilesInDir = allFiles.filter((file) => {
      const normalizedPath = file.toLowerCase();
      return (
        normalizedPath.includes("/js/") &&
        path.basename(normalizedPath) === `${pageName}.js`
      );
    });

    for (const jsFile of pageJsFilesInDir) {
      jsFiles.push({
        path: jsFile,
        content: await fs.readFile(jsFile, "utf-8"),
      });
    ...
    // Check for common JS files in the js directory
    const commonJsFiles = [
      "common.js",
      "script.js",
      "main.js",
    ...
    for (const jsFileName of commonJsFiles) {
      const jsFilePaths = allFiles.filter((file) => {
        ...
        return (
          normalizedPath.includes("/js/") &&
          path.basename(normalizedPath) === jsFileName.toLowerCase()
        );
      });
    ...
    if (cssFiles.length > 0) {
      ...
      if (htmlContent.includes("</head>")) {
        ...
        if (htmlStart !== -1) {
          ...
          if (htmlEndTag !== -1) {
            htmlContent =
              htmlContent.slice(0, htmlEndTag + 1) +
              `\n<head>${cssContent}\n</head>\n` +
              htmlContent.slice(htmlEndTag + 1);
          }
        } else {
...
async function injectComponents(
  ...
  // Handle <!-- INCLUDE COMPONENT: Name --> syntax
  const componentCommentRegex =
    /<!--\s*INCLUDE\s+COMPONENT\s*:\s*([A-Za-z0-9_-]+)\s*-->/gi;
  let match;
  let newContent = htmlContent;
  let componentFound = false;
  ...
  while ((match = componentCommentRegex.exec(htmlContent)) !== null) {
    ...
    if (processedComponents.has(componentName)) {
      newContent = newContent.replace(
        match[0],
        `<!-- Component ${componentName} already included -->`
      );
      continue;
    }

    // Look for components in any location (supporting the new structure)
    const componentPath = allFiles.find((file) => {
      const normalizedPath = file.toLowerCase();
      const basename = path.basename(normalizedPath, ".html");
      // Check for components in any directory
    ...
    if (componentPath) {
      ...
      // Recursively process this component for nested components
      componentContent = await injectComponents(
        componentContent,
        allFiles,
        templateDir,
        processedComponents
  ...
  // Also handle <include-component name="Name"></include-component> syntax
  const componentTagRegex =
    /<include-component[^>]*name=["']([A-Za-z0-9_-]+)["'][^>]*><\/include-component>/gi;
