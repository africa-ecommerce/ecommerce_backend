...
import { productSchema, productVariationsSchema, updateProductSchema, updateProductVariationsSchema } from "../lib/zod/schema";
import {
  deleteImages,
  uploadMiddleware,
  uploadImages,
...
export const productController = {
  ...
  getSupplierProducts: async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      const supplier = req.supplier!;
      // Get all products
...
updateProductStockAndPrice: [
  async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      ...
      let validatedVariations: any = [];
      if (productData.variations && Array.isArray(productData.variations)) {
        const variationsResult = updateProductVariationsSchema.safeParse(productData.variations);
        if (!variationsResult.success) {
